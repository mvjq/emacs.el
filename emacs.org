...
*** Programming
**** Aggressive Indent - CUIDADO: identação agressiva
#+begin_src emacs-lisp
...
**** Zeal ta no ponto (Y)
     Carrego o que tiver no pont para o zeal
#+begin_src emacs-lisp
...
#+end_src
**** Fycheck
     Configurando essa maravilha de flycheck, melhor pessoa (compila
     em background) e avisa por erros e inconsistência.
#+begin_src emacs-lisp
...
*** Hooks
#+begin_src emacs-lisp
(add-hook 'before-save-hook 'delete-trailing-whitespace) ;; remover espaco em branco que sobra
(add-hook 'text-mode-hook 'turn-on-auto-fill) ;; modo auto fil para textos
(add-hook 'after-save-hook 'executable-make-buffer-file-executable-if-script-p)
#+end_src

*** Keybindings(atalhos) e navegação
...
#+end_src
**** Windows (movimentação entre janelas)
#+begin_src emacs-lisp
...
**** Movimentação rápida dentro com arquivo com SHIFT

#+begin_src emacs-lisp

;; KEYBINDINGS (colocar num arquivo melhor)
 ;; Move more quickly
...
**** Pop to mark
     ...
     #+end_src
**** Which Key mode - qual tecla
     modo que mostra um completion para os atalhos :)
#+begin_src emacs-lisp
...
*** Defuns
    ...
    Funções que eu garimpo na internet

#+begin_src emacs-lisp
;; defun for testing emacs


(defun prelude-move-beginning-of-line (arg)
  "Move point back to indentation of beginning of file.
...
*** Emacs
    Funções, modos e configurações para depuração do Emacs
#+begin_src emacs-lisp
;; debug
(setq edebug-trace t)
